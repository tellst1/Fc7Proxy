...
//Start/default URL that that will be proxied when Fc7Proxy is first loaded in a browser/accessed directly with no URL to proxy.
//If empty, Fc7Proxy will show its own landing page.
$startURL = "";

//When no $startURL is configured above, Fc7Proxy will show its own landing page with a URL form field
//and the configured example URL. The example URL appears in the instructional text on the Fc7Proxy landing page,
...
if (version_compare(PHP_VERSION, "5.4.7", "<")) {
  die("Fc7Proxy requires PHP version 5.4.7 or later.");
}

$requiredExtensions = ['curl', 'mbstring', 'xml'];
foreach($requiredExtensions as $requiredExtension) {
  if (!extension_loaded($requiredExtension)) {
    die("Fc7Proxy requires PHP's \"" . $requiredExtension . "\" extension. Please install/enable it on your server and try again.");
  }
}
...
function makeRequest($url) {
  ...
  if (empty($user_agent)) {
    $user_agent = "Mozilla/5.0 (compatible; Fc7Proxy)";
  }
  $ch = curl_init();
  ...
  switch ($_SERVER["REQUEST_METHOD"]) {
    case "POST":
      ...
      //If the Fc7ProxyFormAction field appears in the POST data, remove it so the destination server doesn't receive it.
      $postData = Array();
      parse_str(file_get_contents("php://input"), $postData);
      if (isset($postData["Fc7ProxyFormAction"])) {
        unset($postData["Fc7ProxyFormAction"]);
      }
...
if (isset($_POST["Fc7ProxyFormAction"])) {
  $url = $_POST["Fc7ProxyFormAction"];
  unset($_POST["Fc7ProxyFormAction"]);
} else {
  $queryParams = Array();
  parse_str($_SERVER["QUERY_STRING"], $queryParams);
  //If the Fc7ProxyFormAction field appears in the query string, make $url start with its value, and rebuild the the query string without it.
  if (isset($queryParams["Fc7ProxyFormAction"])) {
    $formAction = $queryParams["Fc7ProxyFormAction"];
    unset($queryParams["Fc7ProxyFormAction"]);
...
if (empty($scheme)) {
  ...
  if (strpos($url, "//") === 0) {
    ...
    die('Error: Detected a "' . $scheme . '" URL. Fc7Proxy exclusively supports http[s] URLs.');
}
...
if (stripos($contentType, "text/html") !== false) {
  ...
  foreach($xpath->query("//form") as $form) {
    ...
    $actionInput->appendXML('<input type="hidden" name="Fc7ProxyFormAction" value="' . htmlspecialchars($action) . '" />');
  ...
  echo "<!-- Proxified page constructed by Fc7Proxy -->\n" . $doc->saveHTML();
